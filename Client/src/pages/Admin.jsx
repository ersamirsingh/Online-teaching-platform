   ...
   const adminActions = {
      courses: [
         ...
         },
         {
            ...
            color: 'btn-secondary',
            action: () => navigate('/viewcourses'),
         },
         {
            title: 'Update Courses',
            color: 'bg-yellow-400',
            action: () => setActiveTab('update-course'),
         },
      ],
      lessons: [
         {
            ...
            color:'btn-primary',
            action: () => setActiveTab('add-lesson'),
         },
         {
            title: 'Manage Lessons',
            color: 'btn-secondary',
            action: () => setActiveTab('manage-lessons'),
         },
         {
            title: 'View Course Lessons',
            color: 'bg-yellow-400',
            action: () => setActiveTab('view-lessons'),
         },
      ],
      quizzes: [
         {
   ...
   return (
      <div className="min-h-screen bg-base-200">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="card shadow-xl group bg-white/15 backdrop-blur-xl rounded-3xl p-6 border border-white/20 transform transition-all duration-500 hover:scale-105 hover:bg-white/25 hover:shadow-2xl cursor-pointer">
               <div className="card-body">
               <h2 className="card-title text-secondary">Lesson Management</h2>
               <div className="space-y-4">
                  {adminActions.lessons.map((action, index) => (
                     <button
                        key={index}
                        className={`btn ${action.color} w-full`}
                        onClick={action.action}
